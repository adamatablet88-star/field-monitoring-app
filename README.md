...
"טפסי שטח" (עדשת דלק, SVE, Bio-venting — בחירת אתר, ובאתר "משולב" גם
בחירת פרוטוקול) ו-"הקמת אתר" (לקוחות/אתרים/קידוחים/מיכלים/מערכות
טיפול/פרמטרים). אין עדיין התחברות אמיתית — ראו `docs/roadmap.md`.
...
שלבים 1–5 ב-roadmap הושלמו: שלד הפרויקט, סנכרון בסיסי (outbox + push/
pull עם זיהוי קונפליקטים מבוסס גרסאות), מסך הקמת אתר מלא, וטפסי שטח
מלאים לשלושה מודולים — עדשת דלק, SVE (טיפול קטן/גדול, קידוחי טיפול,
חישוב יעילות חי, TO-15), ו-Bio-venting (נקודות ניטור בעומקים משתנים,
מבחן חמצן שנתי). מנוע הפרמטרים הגמיש מהקמת האתר פעיל גם בטפסי SVE/
Bio-venting — כל פרמטר שהמנהל מגדיר מקבל שדה + באנר סף קריטי בטופס
אוטומטית, בלי שינוי קוד. כל הכתיבה — הקמת אתר וגם ביקורי שטח — עוברת
דרך אותו מנגנון סנכרון offline-first. נבדק מקצה-לקצה בדפדפן אמיתי מול
Postgres אמיתי בכל שלב; תוך כדי הבדיקות נמצאו ותוקנו באג sync אמיתי
ובאג UX (תוויות שדות מתנגשות) — ראו `docs/roadmap.md`. השלב הבא הוא
טופס דיגום מי תהום.
