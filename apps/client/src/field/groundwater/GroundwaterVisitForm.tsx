...
import type { GroundwaterVisit, GroundwaterWell, StabilizationReading } from "@field-monitoring/shared";
import { db } from "../../db";
...
export function GroundwaterVisitForm({ well, existingVisit, onDone }: GroundwaterVisitFormProps) {
  const { save } = useLocalCollection<GroundwaterVisit>(db.groundwaterVisits, "groundwaterVisit");
  ...
  const [samplingDepth, setSamplingDepth] = useState(existingVisit?.suggestedSamplingDepth?.toString() ?? "");
  const [samplingDepthTouched, setSamplingDepthTouched] = useState(!!existingVisit);
  ...
  useEffect(() => {
    if (!samplingDepthTouched && waterDepthNum !== null) {
      setSamplingDepth((waterDepthNum + 1).toFixed(2));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [waterDepthNum]);

  const columnHeight = waterDepthNum !== null ? Math.max(0, well.wellDepth - waterDepthNum) : null;
  // Well volume from depth/diameter (docs/data-model.md 11.2): standard purge-volume
  // calculation for a cylindrical casing. Diameter assumed given in inches, depth in
  // meters, per the "10 אינץ'" example in docs/data-model.md section 2.
        ...
        <fieldset>
          ...
          <label>
            ...
            <input
              ...
              value={samplingDepth}
              onChange={(e) => {
                setSamplingDepthTouched(true);
                setSamplingDepth(e.target.value);
              }}
            />
          </label>
        </fieldset>
