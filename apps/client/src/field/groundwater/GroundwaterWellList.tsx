...
import type { GroundwaterVisit, GroundwaterWell } from "@field-monitoring/shared";
import { db } from "../../db";
...
export function GroundwaterWellList({ siteId }: GroundwaterWellListProps) {
  const { items: allWells } = useLocalCollection<GroundwaterWell>(db.groundwaterWells, "groundwaterWell");
  const { items: allVisits } = useLocalCollection<GroundwaterVisit>(db.groundwaterVisits, "groundwaterVisit");
