...
import type { ParameterReading, SveSystemVisit, SveVisitType, SveWellVisit, TreatmentSystem, TreatmentWell } from "@field-monitoring/shared";
import { db } from "../../db";
...
import { emptyNotMeasuredDraft, notMeasuredFieldToDraft, draftToNotMeasuredField } from "../notMeasured";
...
function emptyWellDraft(): SveWellDraft {
  return {
    vacuum: emptyNotMeasuredDraft,
    pid: emptyNotMeasuredDraft,
    waterDepth: emptyNotMeasuredDraft,
    productDepth: emptyNotMeasuredDraft,
    bottomDepth: emptyNotMeasuredDraft,
  };
}
...
export function SveVisitForm({ system, onDone }: SveVisitFormProps) {
  const { items: visits, save } = useLocalCollection<SveSystemVisit>(db.sveSystemVisits, "sveSystemVisit");
  const { items: allTreatmentWells } = useLocalCollection<TreatmentWell>(db.treatmentWells, "treatmentWell");
  ...
  const [wellDrafts, setWellDrafts] = useState<Record<string, SveWellDraft>>(() => {
    const map: Record<string, SveWellDraft> = {};
    treatmentWells.forEach((well) => {
      map[well.id] = wellDraftFromVisit(existingVisit?.wellVisits.find((wv) => wv.treatmentWellId === well.id));
    });
    return map;
  });
  ...
  useEffect(() => {
    setWellDrafts((prev) => {
      const next = { ...prev };
      let changed = false;
      for (const well of treatmentWells) {
        if (!next[well.id]) {
          next[well.id] = wellDraftFromVisit(existingVisit?.wellVisits.find((wv) => wv.treatmentWellId === well.id));
          changed = true;
        }
      }
  ...
  }, [treatmentWells.map((w) => w.id).join(",")]);
    ...
    const payload: SveSystemVisit = {
      ...
      wellVisits: showWellForms
        ? treatmentWells.map((well) => {
            const draft = wellDrafts[well.id] ?? emptyWellDraft();
            return {
          ...
          <fieldset>
            ...
            {treatmentWells.map((well) => (
              <SveWellVisitFields
                key={well.id}
                well={well}
                draft={wellDrafts[well.id] ?? emptyWellDraft()}
                onChange={(patch) =>
                  setWellDrafts((prev) => ({ ...prev, [well.id]: { ...(prev[well.id] ?? emptyWellDraft()), ...patch } }))
                }
              />
            ))}
          </fieldset>
