...
import type { EvacuationMethod, FuelLensVisit, Site, Tank, Well } from "@field-monitoring/shared";
...
/**
 * Builds the quarterly regulatory-report workbook exactly as specified in
 * docs/business-logic.md 5.4: a structured Excel export (not an automated
 * combined document), so the hand-written analysis section of the real
 * report stays separate.
 */
export async function buildFuelLensWorkbook(params: {
  ...
  for (const visit of quarterlyVisits.sort((a, b) => a.visitDate.localeCompare(b.visitDate))) {
    ...
    // "עובי עדשה (מודגש מעל 0.5 מ')" — docs/business-logic.md 5.4.
