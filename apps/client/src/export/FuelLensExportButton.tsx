...
import type { FuelLensVisit, Site, Tank, Well } from "@field-monitoring/shared";
import { db } from "../db";
import { useLocalCollection } from "../admin/useLocalCollection";
import { quarterKey } from "../trends/timeRange";

interface FuelLensExportButtonProps {
  siteId: string;
}

/** Structured Excel export matching the quarterly regulatory report exactly — docs/business-logic.md 5.4. */
export function FuelLensExportButton({ siteId }: FuelLensExportButtonProps) {
  ...
  const { items: tanks } = useLocalCollection<Tank>(db.tanks, "tank");
  const { items: visits } = useLocalCollection<FuelLensVisit>(db.fuelLensVisits, "fuelLensVisit");
  ...
  async function handleExport() {
    ...
    setExporting(true);
    try {
      // exceljs is sizeable (~900KB) and only ever needed here — loaded on
      // demand so the rest of the app doesn't pay for it upfront.
