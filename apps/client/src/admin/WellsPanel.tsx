...
import type { RecoveryMethod, Well, Tank } from "@field-monitoring/shared";
import { db } from "../db";
...
export function WellsPanel({ siteId }: WellsPanelProps) {
  const { items: allWells, save, remove } = useLocalCollection<Well>(db.wells, "well");
  const { items: allTanks } = useLocalCollection<Tank>(db.tanks, "tank");
