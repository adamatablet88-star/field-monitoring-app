...
import type { CriticalDirection, ParameterConfig } from "@field-monitoring/shared";
import { db } from "../db";
...
export function ParametersPanel({ systemId }: ParametersPanelProps) {
  const { items: allParameters, save, remove } = useLocalCollection<ParameterConfig>(db.parameterConfigs, "parameterConfig");
    ...
    <section className="panel nested">
      ...
      {showForm ? (
        <form onSubmit={handleSubmit} className="inline-form stacked">
          ...
          <fieldset>
            <legend>סף קריטי (עצמאי מהטווח התקין — ראו docs/data-model.md §8)</legend>
            <label>
              כיוון
              <select
