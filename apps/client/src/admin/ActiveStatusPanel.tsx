...
import type { ActiveStatus, TreatmentSystem } from "@field-monitoring/shared";
import { db } from "../db";
...
export function ActiveStatusPanel({ siteId }: ActiveStatusPanelProps) {
  const { items: allStatuses, save } = useLocalCollection<ActiveStatus>(db.activeStatuses, "activeStatus");
  ...
  async function handleSubmit(e: React.FormEvent, row: ScopeRow, existing: ActiveStatus | null) {
    ...
    // Mirrors what the server will stamp (see stampSource in
    // apps/server/src/sync/router.ts), so the optimistic local write reads
    // right immediately — the server re-stamps from the authenticated
    // user's role regardless, so this value is never actually trusted.
    const source = getStoredUser()?.role === "admin" ? "admin" : "technician";
