...
import type { Tank, Well } from "@field-monitoring/shared";
import { db } from "../db";
...
export function TanksPanel({ siteId }: TanksPanelProps) {
  const { items: allTanks, save, remove } = useLocalCollection<Tank>(db.tanks, "tank");
  const { items: allWells } = useLocalCollection<Well>(db.wells, "well");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await save({ id: newId(), siteId, label: label.trim(), wellIds: [] });
    setLabel("");
    setShowForm(false);
  }
  ...
  return (
    <section className="panel">
      ...
      <p className="hint">
        שיוך קידוח למיכל נעשה מתוך טופס הקידוח (אמצעי פינוי: סקימר אקטיבי) — לא כאן.
      </p>
